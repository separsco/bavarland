...
function SessionTime({ time }: { time: string }) {
  ...
  return (
    <span
      dir="ltr"
      className="inline-flex items-center gap-1 text-[11px] bg-[#F2F2F3] text-muted rounded-full px-2.5 py-1"
    >
    ...
    <section className="space-y-4">
      ...
      <div className="-mx-1 flex gap-2 overflow-x-auto px-1">
        {planningWeekDays.map((day) => (
          <div
            key={day.id}
            className={`flex min-w-[4.5rem] shrink-0 flex-col items-center rounded-lg border px-3 py-2 ${day.active
